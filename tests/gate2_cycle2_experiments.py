...
from tests.gate2_next_cycle_experiments import (
    START, END, FEE_GRID, DELAYS, BASE as OLD_BASE,
    months, fetch, parse_valid_bars, continuous_segments,
    event_return, concentration_and_dd, sha256_file,
)
...
def event_condition(hyp,i,bars,p):
 ...
 if hyp=='HYP-0009':
  ...
  if current is None or len(history)<48: return False
 ...
 if hyp=='HYP-0010':
  ...
  if v==0: return True
  base=abs_median_previous(bars,i,24)
  return base is not None and bars[i].volume>=v*base*Decimal('100')
...
def loo(detail):
 vals=[]
 for d in detail:
  if d['aggregate'] and d['aggregate']['events']: vals.append(Decimal(d['aggregate']['compound_return']))
...
def benchmark(segments,fee,slip):
 ...
 for seg in segments:
  ...
  bh.append((sell/buy-1)*(Decimal('1')-fee)*(Decimal('1')-fee))
...
def main():
 ...
 report={'experiment_version':'gate2-cycle2-v1','code_commit':os.environ.get('GITHUB_SHA','UNKNOWN'),'preregistration':{'path':str(PREREG),'sha256':prereg_sha},'development_window':[START.isoformat(),END.isoformat()],'validation_or_oos_accessed':False,'registered_parameter_cell_count':54,'assets':{},'decision':'UNASSESSED'}
 import tempfile
 with tempfile.TemporaryDirectory(prefix='gate2-cycle2-') as td:
  ...
  for symbol,asset in loaded.items():
   ...
   for hyp in GRID:
    params_list=list(cartesian(GRID[hyp])); ad['parameter_cells'][hyp]=[]
    for n,p in enumerate(params_list):
     det=detail_segments(segs,hyp,p,Decimal('0.001'),Decimal('0.0005'),1); s=summary(det)
     ad['parameter_cells'][hyp].append({'cell':f'parameter_{n:03d}','params':{k:str(v) for k,v in p.items()},'summary':s['aggregate'],'errors':[d['error'] for d in det if d['error']]})
    bp=BASE[hyp]; base_detail=detail_segments(segs,hyp,bp,Decimal('0.001'),Decimal('0.0005'),1); ad['regime'][hyp]=regime(base_detail,segs); ad['leave_one_segment_out'][hyp]=loo(base_detail); ad['concentration_drawdown'][hyp]=concentration_and_dd(base_detail); ad['fee_slippage'][hyp]=[]
    for label,fee,slip in FEE_GRID:
     det=detail_segments(segs,hyp,bp,fee,slip,1); ad['fee_slippage'][hyp].append({'fee_case':label,'commission':str(fee),'slippage':str(slip),'summary':summary(det)['aggregate']})
    ad['timing'][hyp]=[]
    for delay in DELAYS:
     det=detail_segments(segs,hyp,bp,Decimal('0.001'),Decimal('0.0005'),delay); ad['timing'][hyp].append({'delay_bars':delay,'summary':summary(det)['aggregate']})
   report['assets'][symbol]=ad
 out=Path('gate2_cycle2_results'); out.mkdir(exist_ok=True); (out/'cycle2_results.json').write_text(json.dumps(report,separators=(',',':')))
 print('Gate 2 Cycle 2 evidence generated')
