...
def condition(h,i,b,p):
    if h=='HYP-0020': return i>=1 and b[i].timestamp.hour==p['hour'] and b[i].close>b[i].open and ret(b,i)>=p['threshold'] and location(b[i])>=p['location']
    if h=='HYP-0021':
        k=p['k']
        if i<k+1: return False
        rs=[ret(b,j) for j in range(i-k,i)]
        return all(x>0 for x in rs) and b[i-1].close/b[i-k].close-1>=p['threshold'] and location(b[i])<=p['location']
    if h=='HYP-0022':
        if i<1: return False
        r=b[i].high-b[i].low
        if r<=0: return False
...
def simulate(seg,h,p,fee,slip,delay):
    ...
    while i+delay<len(seg):
        if condition(h,i,seg,p):
            e=i+delay; r=event_return(seg[e],seg[e],fee,slip); events.append({'signal_index':i,'entry_index':e,'exit_index':e,'signal_timestamp':seg[i].timestamp.isoformat(),'entry_timestamp':seg[e].timestamp.isoformat(),'return':str(r)}); i=e+1
        else: i+=1
    return events
def compound(rs):
    g=Decimal('1')
...
def detail(segs,h,p,fee,slip,delay):
    ...
    for i,s in enumerate(segs):
        try:
            ev=simulate(s,h,p,fee,slip,delay); rs=[Decimal(x['return']) for x in ev]; out.append({'segment':i,'aggregate':{'events':len(ev),'compound_return':str(compound(rs))},'events':ev,'error':None})
...
def benchmark(segs,fee,slip):
    ...
    for s in segs:
        if len(s)>1:
            buy=s[0].open*(Decimal('1')+slip); sell=s[-1].close*(Decimal('1')-slip); rs.append(sell/buy*(Decimal('1')-fee)*(Decimal('1')-fee)-1)
