...
def _authorized_manifest(candidate):
    return {
        "status": "AUTHORIZED_FOR_FIRST_DEVELOPMENT_V2_EXECUTION",
...
def _install(monkeypatch, candidate, changed=()):
    manifest = _authorized_manifest(candidate)
    freeze = _freeze(candidate)
    gate = _authorized_gate()
    monkeypatch.setattr(
        lock, "assert_ams_dep_empirical_release_allowed", lambda: gate
    )
    monkeypatch.setattr(
        lock, "load_development_execution_manifest", lambda: manifest
    )
    monkeypatch.setattr(
        lock, "load_development_implementation_freeze", lambda: freeze
    )
    monkeypatch.setattr(
        lock, "verify_frozen_implementation", lambda value: {}
    )
    monkeypatch.setattr(
        lock,
...
def test_exact_authorized_v2_contract_passes(monkeypatch):
    candidate = "a" * 40
    executing = "b" * 40
    _install(
        monkeypatch,
    ...
    assert result["review_anchor"]["sha"] == candidate
    assert result["executing_sha"] == executing


def test_mutable_v2_governance_cannot_rebind_reviewed_candidate(monkeypatch):
    actually_reviewed = "a" * 40
    malicious_rebind = "c" * 40
    _install(
        monkeypatch,
        malicious_rebind,
        changed=tuple(lock.ALLOWED_POST_CANDIDATE_PATHS),
    )
    backend = FixedRefBackend(
        {lock.DEFAULT_REVIEW_ANCHOR_REF: actually_reviewed}
    )
    with pytest.raises(
        lock.DevelopmentExecutionV2Error,
        match="anchor target mismatch",
    ):
...
def test_canonical_downstream_permissions_remain_closed(
    monkeypatch, field
):
    candidate = "a" * 40
    _, _, gate = _install(monkeypatch, candidate)
    gate[field] = True
    backend = FixedRefBackend(
        {lock.DEFAULT_REVIEW_ANCHOR_REF: candidate}
    )
