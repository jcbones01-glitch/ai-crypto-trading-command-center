...
- Lo–MacKinlay-inspired variance-ratio diagnostics q=2, 6, 24, on unconditioned consecutive hourly returns only. Never concatenate a state-filtered sequence and call it a random walk. The source-audited finite-sample point-estimate formula is specified in `AMS_DEP_NUMERICAL_CONTRACT_V1.md`. VR inference is deferred: no VR p-values. Synthetic implementation may proceed; actual-return execution remains blocked.
...
## Draft revision: numerical core and synthetic-only specification

`AMS_DEP_NUMERICAL_CONTRACT_V1.md` resolves the source-audited descriptive VR formula, SVD/conditioning rules, finite-sample covariance correction, interval and quantile conventions, exact-hour HAC, restriction matrices and missing-slot multiplicity semantics. `AMS_DEP_SYNTHETIC_CALIBRATION_V1.md` and its fixed JSON define a synthetic numerical-core pilot before results. This is a recorded pre-empirical draft revision, not an amendment to a frozen market-data experiment.

Release-blocker progress: the VR point-estimate equation audit and numerical choices are specified; numerical-core verification may proceed without actual returns. Full endogenous AMS-V1 calibration, complete empirical row/support/secondary-output implementation, protected-read enforcement, exhaustive empirical output inventory and independent design/implementation approval remain open. No reviewer has approved this draft as of the revision. Existing support thresholds, states, primary model and six-test family remain unchanged.
