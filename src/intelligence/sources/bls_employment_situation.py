...
SOURCE_ID = "bls/employment-situation-news-release"
SOURCE_VERSION = "bls-employment-situation-html-v1"
ARCHIVE_INDEX_URL = "https://www.bls.gov/bls/news-release/empsit.htm"
_REQUIRED_TITLE = "Employment Situation News Release"

_ARCHIVE_PATH_RE = re.compile(
    r"^/news\.release/archives/empsit_\d{8}\.htm$",
    re.IGNORECASE,
)
_EMBARGO_RE = re.compile(
    r"embargoed\s+until.{0,96}?"
...
class _ArchiveLinkParser(HTMLParser):
    ...
    def handle_endtag(self, tag: str) -> None:
        ...
        if _ARCHIVE_PATH_RE.match(parsed.path) and "employment situation" in label.lower():
...
def discover_employment_situation_urls(
    raw_index_payload: bytes,
    *,
    index_url: str = ARCHIVE_INDEX_URL,
) -> tuple[str, ...]:
    """Discover archived HTML Employment Situation releases from official BLS."""
...
def _validate_release_identity(text: str) -> None:
    if _REQUIRED_TITLE.lower() not in text.lower():
        raise ValueError("page is not identified as an Employment Situation News Release")
    if "embargoed until" not in text.lower():
        raise ValueError("Employment Situation embargo statement not found")
...
def _parse_embargo_timestamp(text: str) -> datetime:
    ...
    if not match:
        raise ValueError(
            "explicit Employment Situation embargo timestamp with EST/EDT/ET not found"
        )
    ...
    if not 1 <= hour <= 12 or not 0 <= minute <= 59:
        raise ValueError("invalid Employment Situation embargo time")
...
def parse_employment_situation_release(
    raw_payload: bytes,
    *,
    source_url: str,
    assets: tuple[str, ...],
    ...
    """Parse one official BLS Employment Situation release.

    V1 certifies only release identity and embargo timing. It does not interpret
    payroll values, unemployment values, revisions, forecasts, or surprise.
    """
    ...
    if payload_format == "html":
        ...
        version = "bls-employment-situation-rendered-text-v1"
    else:
        raise ValueError("unsupported Employment Situation payload representation")
    ...
    return EventRecord(
        event_id=event_id,
        event_type="us_employment_situation_release",
        region="US",
        assets=assets,
