"""AMS-DEP Development empirical recovery runner V3.

V3 adds only the approved source-coverage certification overlay after the unchanged V2 treatment-aware source-normalization seam. The primary
sample and statistical path remain identical to the approved V1 execution
contract. This runner is inert unless every V3 governance lock and durable ref
...
from research_core.ams_dep_development_execution_lock_v3 import (
    DEFAULT_CLAIM_REF,
...
from research_core.ams_dep_empirical_access_v3 import load_development_source
...
SPEC_PATH = ROOT / "docs/AMS_DEP_DEVELOPMENT_EMPIRICAL_EXECUTION_SPEC_V3.md"
REGISTRATION_PATH = ROOT / "research/governance/ams_dep_development_execution_v3.json"
OUTPUT_DIR = ROOT / "research/experiments/ams_dep_development_empirical_v3"
...
def _ambient_execution_context() -> dict:
    return {
        ...
        "confirmation_token": "AMS_DEP_DEVELOPMENT_EMPIRICAL_V3",
        "confirmation_verified":
            os.environ.get("AMS_DEP_DEVELOPMENT_V3_CONFIRMATION_VERIFIED")
            == "true",
        "forwarded_claim_ref":
            os.environ.get("AMS_DEP_DEVELOPMENT_V3_CLAIM_REF"),
        "forwarded_claim_sha":
            os.environ.get("AMS_DEP_DEVELOPMENT_V3_CLAIM_SHA"),
...
def _failure_result(exc: Exception) -> dict:
    return {
        "classification": "AMS_DEP_DEVELOPMENT_EMPIRICAL_V3_INTEGRITY_FAIL",
        "version": 3,
        "parent_v1_incident": dict(PARENT_V1),
        "parent_v2_incident": dict(PARENT_V2),
...
def run() -> dict:
    ...
    INCIDENT_STATE["stage"] = "DURABLE_V3_CLAIM_VERIFICATION"
    ...
    INCIDENT_STATE["stage"] = "POST_V3_CLAIM_PRE_SOURCE"
    ...
    if tuple(raw_p_by_slot) != SLOTS:
        raise RuntimeError("V3 empirical primary family order changed")
    ...
    classification = (
        ...
        if invalidity_fail
        else "AMS_DEP_DEVELOPMENT_EMPIRICAL_V3_EXECUTED"
    )
...
def main() -> None:
    ...
    try:
        ...
        print("AMS_DEP_DEVELOPMENT_EMPIRICAL_V3_INTEGRITY_FAIL")
